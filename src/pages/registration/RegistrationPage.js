...
const RegistrationPage = () => {
  ...
  const handleNext = async () => {
    ...
    if (step === 1 && validateStep1()) {
      ...
      const isBirthCert = cleanedInput.length === 17;
      const isNid = [10, 13].includes(cleanedInput.length);

      const createOtpData = {
        ...(isNid && { NID: cleanedInput }),
        ...(isBirthCert && { birthCertificateNo: cleanedInput }),
        ...
        <Paper className={classes.paper} elevation={3}>
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            <Box mt={2} className={classes.inputContainer}>
              ...
              {/* Back Button */}
              <Button fullWidth onClick={() => history.push("/")} startIcon={<ArrowBackIcon />} color="primary" variant="text" style={{ marginTop: 8 }}>
                পিছনে
              </Button>
            </Box>
          </form>
