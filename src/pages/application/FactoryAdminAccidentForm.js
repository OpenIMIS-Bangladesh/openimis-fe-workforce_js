...
const FactoryAdminAccidentForm = ({ handleChange, formData, setFormData, applicationType, errors }) => {
  ...
  useEffect(() => {
    const metadataDeathDate = formData?.metadata?.deathDate;
    const accidentDeathDate = formData?.employeeAccidentInfo?.dateOfDeath;
    ...
    if (metadataDeathDate && !accidentDeathDate) {
      handleChange("dateOfDeath", metadataDeathDate);
    }
  }, []);
          ...
          <Grid container spacing={2}>
              ...
              <FormControl component="fieldset" fullWidth>
                ...
                <RadioGroup row value={selectedDocxAccidentType} onChange={handleDocxAccidentTypeChange}>
                  {Object.values(DOCX_ACCIDENT_TYPES).map((type) => (
                    <FormControlLabel key={type} value={type} control={<Radio color="primary" />} label={<FormattedMessage id={type} />} />
                  ))}
                </RadioGroup>
            ...
            <Grid item xs={6} className={classes.item}>
              <PublishedComponent
                ...
                label={"workforce.employee.accident.info.joiningDate"}
                value={formData?.employeeAccidentInfo?.joiningDate || ""}
                  ...
                  <FormControl component="fieldset">
                    ...
                    >
                      <FormControlLabel
                        value="yes"
                        control={<Radio color="primary" />}
                        label={<FormattedMessage id="workforce.application.permission.yes" />}
                      />
            ...
            {otherInjured === "yes" && (
              <Grid item xs={12} className={classes.item}>
                ...
                <Typography variant="subtitle2" gutterBottom>
                  {<FormattedMessage id="workforce.accident.otherInjuredName.label" />}
                </Typography>
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <TextInput
                      label="workforce.employee.name"
                      value={formData?.employeeAccidentInfo?.otherInjuredName || ""}
                      onChange={(v) => handleChange("otherInjuredName", v)}
                    />
                  </Grid>
                  <Grid item xs={6}>
                    <TextInput
                      label="workforce.employee.designation"
                      value={formData?.employeeAccidentInfo?.otherInjuredDesignation || ""}
                      onChange={(v) => handleChange("otherInjuredDesignation", v)}
                    />
                  </Grid>
                  <Grid item xs={6}>
                    <TextInput
                      label="workforce.employee.phone"
                      value={formData?.employeeAccidentInfo?.otherInjuredPhone || ""}
                      onChange={(v) => handleChange("otherInjuredPhone", v)}
                    />
                  </Grid>
                  <Grid item xs={6}>
                    <TextInput
                      label="workforce.accident.otherInjuredType.label"
                      value={formData?.employeeAccidentInfo?.otherInjuredType || ""}
                      onChange={(v) => handleChange("otherInjuredType", v)}
                    />
                  </Grid>
                  <Grid item xs={6} className={classes.item}>
                    <FormControl component="fieldset" fullWidth>
                      ...
                      >
                        ...
                        <FormControlLabel
                          value="no"
                          control={<Radio color="primary" />}
                          label={<FormattedMessage id="workforce.application.permission.no" />}
                        />
                      </RadioGroup>
                    </FormControl>
                  </Grid>
            ...
            {isAdmitted === "yes" && (
              <Grid item xs={12}>
                {hospitalList.map((hospital, index) => (
                  <Box key={index} mb={2} p={2} border={1} borderColor="grey.300" borderRadius={4} style={{ backgroundColor: "#fafafa" }}>
                    <Grid container spacing={2}>
                ...
                <Typography style={{ color: "red" }}>
                  **
                  <FormattedMessage id="workforce.add.hospital.info" />
                  **
                </Typography>
                {/* ADD BUTTON */}
                <Box mt={1}>
                  <Button variant="outlined" color="primary" startIcon={<AddIcon />} onClick={handleAddHospital}>
