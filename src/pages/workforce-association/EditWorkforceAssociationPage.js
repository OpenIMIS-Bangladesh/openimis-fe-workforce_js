...
import { TextInput, journalize, PublishedComponent, FormattedMessage, formatMutation, decodeId, withHistory } from "@openimis/fe-core";
...
class EditWorkforceAssociationPage extends Component {
  ...
  componentDidUpdate(prevProps) {
    const previousAssociation = prevProps.workforceAllAssociation;
    const currentAssociation = this.props.workforceAllAssociation;

    if (currentAssociation && currentAssociation !== previousAssociation && currentAssociation.id) {
      this.setState({
        stateEdited: { ...currentAssociation },
        isSaved: false,
      });
    }
  ...
  save = async () => {
    ...
    try {
      await dispatch(updateWorkforceAssociation(workforceAssociationData, `Update Workforce Association ${workforceAssociationData.nameEn}`)).then((res)=>window.location.reload())
      this.setState({ isSaved: true });
    } catch (error) {
      console.error("Failed to update workforce association:", error);
    }
  };
  ...
  render() {
    ...
    console.log("Redux association:", this.props.workforceAllAssociation);
    console.log("Form state:", this.state.stateEdited);
            ...
            <Paper className={classes.paper}>
              <Grid container className={classes.tableTitle}>
                <Grid item xs={12} className={classes.tableTitle}>
                  <Typography>
                    <FormattedMessage module={MODULE_NAME} id="Workforce Association" values={{ label: EMPTY_STRING }} />
                  </Typography>
                </Grid>
              </Grid>
              <Divider />
...
const mapStateToProps = (state) => ({
  workforceAllAssociation:
    state.workforce.workforceAllAssociations?.[0] || {},
  submittingMutation: state.workforce.submittingMutation,
  mutation: state.workforce.mutation,
});

export default withHistory(connect(mapStateToProps)(withStyles(styles)(EditWorkforceAssociationPage)));
