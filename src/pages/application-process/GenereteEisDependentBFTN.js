  ...
  const exportDisabilityExcel = async (eisPayments) => {
    ...
    eisPayments.forEach((row, index) => {
      ...
      const excelRow = sheet.addRow([
        ...
        Number(row?.eisCalculatedAmount || 0).toFixed(2),
        Number(row?.eisApprovedAmount || 0).toFixed(2),
        row?.eisPaymentType || "",
        row?.approvalStatus || "",
        "",
      ]);
    ...
    const totalRow = sheet.addRow(["Total", "", "", ``, Number(totalMonthly || 0).toFixed(2), Number(totalNet || 0).toFixed(2), "", "", "", "", ""]);
                    ...
                    {eisPayments.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        {[WORKFORCE_USER_TYPE.EIS_ASSOCIATION_COMMITTEE, WORKFORCE_USER_TYPE.EIS_COMMITTEE].includes(user_type)
                          ...
                          : (
                            <>
                              <TableCell>{Number(row?.eisCalculatedAmount || 0).toFixed(2)}</TableCell>
                              <TableCell>{Number(row?.eisApprovedAmount || 0).toFixed(2)}</TableCell>
                            </>
                          )
                        }
                        <TableCell>{Number(row?.eisInitialMonthlyAmount || 0).toFixed(2)}</TableCell>
                        <TableCell>{Number(row?.eisMonthlyAmount || 0).toFixed(2)}</TableCell>
                        <TableCell>
