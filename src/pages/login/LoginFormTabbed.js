    ...
    <>
      ...
      <SharedTabPanel value={loginType} index={0}>
        <>
          <Grid item style={{ marginTop: "1rem" }}>
            <TextInput required label="মোবাইল নাম্বার" fullWidth defaultValue="" onChange={(mobileNumber) => setInput("mobile_number", mobileNumber)} />
          </Grid>
          ...
          {!isOtpSent && (
            <Grid item style={{ marginTop: "1rem" }}>
              <Button fullWidth type="button" disabled={mobileNumber.length !== 11} color="primary" variant="contained" onClick={() => sendOtp()}>
                ওটিপি প্রেরণ করুন
              </Button>
            </Grid>
          )}
      ...
      <SharedTabPanel value={loginType} index={1}>
        <>
          ...
          <Grid item style={{ marginTop: "1rem" }}>
            <TextInput required type="password" label="পাসওয়ার্ড" fullWidth onChange={(password) => setCredentials({ ...credentials, password })} />
          </Grid>
          {serverResponse?.message && (
            <Grid item style={{ marginTop: "1rem" }}>
              <Box color="error.main">{getErrorMessage(serverResponse.message)}</Box>
