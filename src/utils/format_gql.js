...
export function formatWorkforceEmployeeGQL(employee) {
  return `
    ${employee?.id ? `id: "${formatGQLString(employee.id)}"` : ""}
    ${employee.workforceFactoryId ? `workforceFactoryId: "${employee.workforceFactoryId}"` : ""}
...
export function formatApplicationeGQL(application) {
  ...
  ${application?.doctorEntries ? `doctorsEntry: ${escapeQuotes(application?.doctorEntries)}` : ""}
  ${application?.institutionInfo ? `institutionInfo: ${escapeQuotes(application?.institutionInfo)}` : ""}
