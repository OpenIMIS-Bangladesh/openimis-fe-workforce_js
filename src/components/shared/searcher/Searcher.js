...
import { cacheFilters, resetCacheFilters, saveCurrentPaginationPage } from "../../../searcherActions";
import { withModulesManager, useTranslations,InfoButton,formatSorter,sort, istoryPush, Contributions,SearcherPane,formatMessage, journalize, FormattedMessage, formatMessageWithValues,ProgressOrError,SearcherExport } from "@openimis/fe-core";
...
class SelectionMenu extends Component {
  ...
  render() {
    ...
    if (!!selectionCount) {
      entries.push({ text:<FormattedMessage module="workforce" id="workforce.clearSelected" />  , action: clearSelected });
    }
    if (!!canSelectAll && canSelectAll(this.props.selection)) {
      entries.push({ text: <FormattedMessage module="workforce" id="workforce.selectAll" />, action: selectAll });
    }
    actions.forEach((a) => {
      if (a.enabled(selection)) {
        entries.push({ text:  <FormattedMessage module="workforce" id={a.label} />, action: a.action });
      }
    });
...
const StyledSelectionMenu = withModulesManager(withTheme(withStyles(styles)(SelectionMenu)));
...
class Searcher extends Component {
  ...
  applyFilters = () => {
    this.setState(
      (state, props) => ({
        ...
        selection: [], // Clear selection when applying filters
      }),
      this._cacheAndApply
    );
  };
  ...
  // Updated to properly handle selection changes
  onChangeSelection = (selection) => {
    this.setState({ selection: Array.isArray(selection) ? selection : [] });
    
    // Call the parent callback if provided
    if (this.props.onCheckBoxSelect) {
      this.props.onCheckBoxSelect(Array.isArray(selection) ? selection : []);
    }
  };
                ...
                <Grid container item alignItems="center" xs={this.isWorker ? 7 : 8} className={classes.paperHeader}>
                  <Grid item xs={8} className={classes.paperHeaderTitle}>
                    <div className={classes.infoSection}>
                      ...
                      <Grid item>
                        {!fetchingItems ? tableTitle : <FormattedMessage module="core" id="core.table.resultsLoading" />}
                      </Grid>
                    </div>
                  </Grid>
...
export default withModulesManager(
  withTheme(withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(Searcher)))
);