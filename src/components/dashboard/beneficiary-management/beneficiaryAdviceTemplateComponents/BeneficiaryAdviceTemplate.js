...
import { createWorkforceEisBankAdvice } from '../../../actions';
import { useDispatch } from "react-redux";
import ReactQuill from "react-quill";
import "react-quill/dist/quill.snow.css";
import { generateTopText, generateBottomText } from '../../../utils/bankAdviceContent';
...
const BeneficiaryAdviceTemplate = ({ open, onClose, paymentData, month, year, fromAdviceList = false }) => {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      setBottomText(generateBottomText());
    }
  }, [open, paymentData]);
  ...
  const handleSaveAdvice = () => {
    if(window.confirm("Are You sure you want to save this Bank Advice?")) {
      let ids = paymentData.map(data => data.id);
      try {
  ...
  return (
    ...
    >
      ...
      <DialogContent className={classes.dialogContent}>
        ...
        <Typography align="right" variant="subtitle1" style={{ fontWeight: "bold", marginBottom: "20px" }}>
...
export default BeneficiaryAdviceTemplate;