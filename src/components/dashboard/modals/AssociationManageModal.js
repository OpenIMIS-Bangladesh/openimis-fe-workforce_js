...
const AssociationManageModal = ({ open, onClose, onSuccess, association }) => {
    console.log("AssociationManageModal Props:", { open, onClose, onSuccess, association });
    const INITIAL_STATE = {
    ...
    const handleSave = () => {
        setLoading(true);
        if(formData.increment > 0 || formData.decrement > 0 || (formData.reason && formData.status)){
            const payload = {
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
            <DialogTitle>Increase or Decrease Beneficiary Payments for <strong>{association?.nameEn} ({association?.shortNameEn})</strong></DialogTitle>
            <Divider />
            <DialogContent>
...
export default AssociationManageModal;