...
import { conditionalEnToBn, getUserTypeFromRights, safeDecodeId } from "../../utils/utils";
...
const PushNotification = () => {
  ...
  const markNotificationAsRead = async (notification, skipNavigation = false) => {
    ...
    try {
      ...
      await dispatch(updateNotification(payload, "updateNotification"));
  ...
  console.log({ emnei: notifications });

  return (
    <>
      <IconButton onClick={handleBellClick} className={classes.defaultIcon} aria-label="show notifications" color="inherit">
        {unreadCount > 0 ? (
      ...
      >
        <Box className={classes.menuHeader}>
          {unreadCount < 1 && <Typography variant="subtitle1">{locale === "en" ? "Notifications" : "নোটিফিকেশন"}</Typography>}
          <Box display="flex" alignItems="center" gap={1}>
            {unreadCount > 0 && (
              <Typography variant="body2">
                {locale === "en" ? unreadCount : Number(unreadCount).toLocaleString("bn-BD")}{" "}
                {locale === "en" ? " Unread Notifications" : " টি নোটিফিকেশন অপঠিত"}
              </Typography>
            )}
            {unreadCount > 0 && (
              <Button
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map((notification) => {
            ...
            return (
              <MenuItem
                key={notification.id}
                className={`${classes.notificationItem} ${read ? classes.notificationRead : classes.notificationUnread}`}
                  ...
                  secondary={
                    <Box>
                      ...
                      {notification.dateCreated && (
                        <Typography variant="caption" display="block" color="textSecondary" className={classes.dateText}>
                          {conditionalEnToBn(notification.dateCreated.split("T")[0], locale)}
                        </Typography>
                      )}
                    </Box>
                  }
